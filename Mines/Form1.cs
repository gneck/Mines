    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            try
            {
                ...
                String[] size = Interaction.InputBox("Velikost? (Max. " + MAX + "x" + MAX + ")", "Hledač min", "10x10").Split('x');

                int x_length = int.Parse(size[0]) < MAX ? int.Parse(size[0]) : MAX;
                int y_length = int.Parse(size[1]) < MAX ? int.Parse(size[1]) : MAX;

                // zjištění počtu min
                int count_of_mines = int.Parse(Interaction.InputBox("Počet min? (menší než " + (x_length * y_length) + ")", "Hledač min", "5"));
